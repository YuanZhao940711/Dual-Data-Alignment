...
class VAERebilder:
    ...
    def _load_vae(self):
        if self.vae is None:
            ...
            # self.vae.eval()
            self.vae.to(device=self.device, dtype=model_dtype)  # 强制所有参数/bias一致 dtype
            self.vae.eval()
...
def read_images_from_dir(dir_path, recursive=True):
    """
    读取目录中的所有图片路径（默认递归子目录）
    """
...
if __name__ == "__main__":
    LIST_FILE = "/root/autodl-tmp/AIGC_Detection/车损AIGC检测图片/真实汽车图片"

    OUTPUT_DIR = "/root/autodl-tmp/AIGC_Detection/车损AIGC检测图片/重建汽车图片"

    # VAE_MODEL_PATH = "/content/drive/MyDrive/PingAn/AIGC_Detection/model_cache/sd2_vae"
    VAE_MODEL_PATH = "/root/autodl-tmp/AIGC_Detection/model_cache/sd2/vae"
    ...
    if not os.path.exists(LIST_FILE):
        ...
        img_paths = read_images_from_dir(LIST_FILE, recursive=True)
        print(f"[INFO] Found {len(img_paths)} images in folder: {LIST_FILE}")
